...
public function ajukanPengembalianWeb($id)
{
    if (!session()->get('logged_in')) {
        return redirect()->to('/login');
    }

    // Hanya anggota yang boleh mengajukan
    ...
    if ($pinjam['status'] !== 'DIPINJAM') {
        return redirect()->back()->with('error', 'Transaksi tidak dapat diajukan');
    }

    // HANYA ubah status, jangan isi tanggal_kembali
    $this->peminjaman->update($id, [
        'status' => 'MENUNGGU_VALIDASI'
    ]);

    return redirect()->to('/riwayat')
        ->with('success', 'Pengembalian berhasil diajukan, menunggu validasi petugas');
}

// Petugas/Admin memvalidasi pengembalian
    public function validasiPengembalianWeb($id)
    {
        if (!session()->get('logged_in')) {
            return redirect()->to('/login');
        }
        ...
        if ($pinjam['status'] !== 'MENUNGGU_VALIDASI') {
            return redirect()->back()->with('error', 'Belum ada pengajuan pengembalian');
        }
        ...
        $denda = 0;
        if ($kembali > $jatuhTempo) {
            $hari = floor(($kembali - $jatuhTempo) / (60 * 60 * 24));
            $denda = $hari * 2000;
        }
